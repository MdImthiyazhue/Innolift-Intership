...
## ML Prediction API (Day 46)

The backend now also serves the Phase 2 **ShopIQ Churn Prediction** model
through Flask, alongside the existing Student Management APIs.

- Model + preprocessing logic: `backend/ml/model.py` (kept fully separate
  from Flask route logic in `app.py`)
- Training script: `backend/ml/train_model.py`
- Endpoint: `POST /api/predict`
- Full write-up: `docs/day-46-ml-integration.md`
...
**Response:**
```json
{ "success": true, "prediction": "Churn", "churn_probability": 0.92 }
```

The model is loaded once at Flask startup (not per-request), input is
validated before it ever reaches the model, and invalid input returns a
`400` with a specific `error` message instead of crashing the server. The
existing `/api/students` endpoints were re-tested after this change and
continue to work unaffected.

```mermaid
flowchart TD
    A[React Frontend] --> B[Flask Backend]
    B --> C[(MySQL: student_management)]
    B --> D[ML Model: ShopIQ Churn Predictor]
    C --> E[JSON Response]
    D --> E
    E --> A
```
...
| Day 46 | ML model (ShopIQ churn prediction) integrated into Flask via `/api/predict` |
